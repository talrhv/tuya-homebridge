...
export default class BaseAccessory {
  constructor(
    ...
    // בגרסה 2.0 Categories נמצא ב-Top Level, ב-1.x הוא בתוך Accessory
    ...
    if (this.homebridgeAccessory) {
      ...
      // יצירת מופע חדש של PlatformAccessory
      ...
      // רישום האביזר - שימוש בשמות מה-Platform כדי למנוע 'undefined'
      ...
      if (pluginName && platformName) {
        ...
        this.log.error(
          "Cannot register accessory: PLUGIN_NAME or PLATFORM_NAME is undefined.",
        );
      }
    }
    ...
    // Default Identify Handler (Async safe)
  ...
  _setupServices(Service, Characteristic) {
    ...
    if (this.subServices.length <= 1) {
      ...
      for (const subName of this.subServices) {
        ...
        const service =
          this.homebridgeAccessory.getService(subName) ||
          this.homebridgeAccessory.addService(
            this.serviceType,
            cleanSubName,
            subName,
