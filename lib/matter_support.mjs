...
export default class TuyaMatterBridge {
  ...
  isValveDevice(device) {
    ...
    // Clean the deviceId to prevent hidden whitespace bugs
    const cleanId = String(deviceId).trim();

    return Boolean(
      (this.platform.config?.options?.valve || []).find((entry) => {
  ...
  async registerDevices(devices = []) {
    ...
    for (const device of devices) {
      ...
      if (!this.pendingRegistrations.has(uuid)) {
  ...
  async syncMessage(message) {
    ...
    if (this.pendingRegistrations.has(uuid)) return;
