...
import SwitchAccessory from "./switch_accessory.mjs";
import WindowCoveringAccessory from "./window_covering_accessory.mjs";
...
export default class TuyaMatterBridge {
  ...
  async registerDevices(devices = []) {
    ...
    for (const device of devices) {
      ...
      if (this.hasAccessoryStructureChanged(existing, created)) {
  ...
  hasAccessoryStructureChanged(existing, created) {
    if (!existing || !created) {
      return false;
    }
    ...
    const category = created.context?.category;
    if (SWITCH_CATEGORIES.has(category)) {
      return SwitchAccessory.hasDifferentMatterShape(existing, created);
    }
  ...
  rebindHandlers(accessory, discoveredDevice) {
    ...
    if (SWITCH_CATEGORIES.has(accessory.context.category)) {
      SwitchAccessory.rebindMatterAccessory(this.platform, accessory, device);
      return;
    }

    accessory.handlers = WINDOW_CATEGORIES.has(accessory.context.category)
      ? WindowCoveringAccessory.buildMatterHandlers(
          this.platform,
          accessory.context,
        )
      : this.buildHandlers(accessory.context, device);
  }
  ...
  createAccessory(device) {
    ...
    if (SWITCH_CATEGORIES.has(category)) {
      return SwitchAccessory.createMatterAccessory(this.platform, device, this);
    }
    if (WINDOW_CATEGORIES.has(category)) {
      return WindowCoveringAccessory.createMatterAccessory(
        this.platform,
        device,
      );
    }
  ...
  buildHandlers(context, discoveredDevice) {
    ...
    if (SWITCH_CATEGORIES.has(category)) {
      return SwitchAccessory.buildMatterHandlers(this.platform, context);
    }

    if (WINDOW_CATEGORIES.has(category)) {
      return WindowCoveringAccessory.buildMatterHandlers(
        this.platform,
        context,
      );
    }
  ...
  async syncDeviceSnapshot(device) {
    ...
    if (OUTLET_CATEGORIES.has(device.category)) {
      await this.syncOnOffState(accessory, device, accessory.context.powerCode);
      return;
    }

    if (SWITCH_CATEGORIES.has(device.category)) {
      await SwitchAccessory.syncMatterState(this, accessory, device);
      return;
    }
    ...
    if (WINDOW_CATEGORIES.has(device.category)) {
      await WindowCoveringAccessory.syncMatterState(this, accessory, device);
    }
  }
  ...
  toBoolean(value, fallback = false) {
    ...
    if (typeof value === "string") {
      return ["true", "1", "on", "opened", "open"].includes(
        value.toLowerCase(),
      );
    }
    return fallback;
