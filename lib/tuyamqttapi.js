...
class TuyaOpenMQ {
  ...
  async _loop_start() {
    while (this.running) {
      ...
      const { url, client_id, username, password, expire_time, source_topic } = mqConfig;
      ...
      client.on('message', (topic, payload) => this._onMessage(topic, payload, mqConfig));
  ...
  _decodeMQMessage(data, password, t) {
    ...
    const decipher = crypto.createDecipheriv('aes-128-gcm', key, iv_buffer);
