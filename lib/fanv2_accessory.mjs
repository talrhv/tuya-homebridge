...
class Fanv2Accessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      if (
        statusMap.code === "switch" ||
        statusMap.code === "fan_switch" ||
        statusMap.code === "switch_fan"
      ) {
      ...
      if (statusMap.code === "mode") {
        ...
        const hbFanState = this.tuyaParamToHomeBridge(
      ...
      if (statusMap.code === "fan_direction") {
        ...
        const hbDirection = this.tuyaParamToHomeBridge(
          Characteristic.RotationDirection,
          this.directionMap.value,
        );
      ...
      if (statusMap.code === "fan_speed_percent") {
        ...
        const value = Math.floor(
          (rawValue * 100 - 100 * this.speed_range.min) /
            (this.speed_range.max - this.speed_range.min),
        ); // 0-100
      ...
      if (statusMap.code === "fan_speed") {
        this.speedMap = statusMap;
        if (
          typeof this.speedMap.value == "string" &&
          this.speedMap.value.constructor == String
        ) {
          ...
          const hbSpeed = parseInt(
            this.speedMap.value * this.speed_coefficient,
          );
      ...
      if (this.lightService && statusMap.code === "bright_value") {
        ...
        this.bright_range = this.getBrightnessFunctionRange(
          this.brightValue.code,
        );
        const rawValue = this.brightValue.value;
        const percentage = Math.floor(
          ((rawValue - this.bright_range.min) * 100) /
            (this.bright_range.max - this.bright_range.min),
        ); //    $
        this.normalAsync(
    ...
    switch (name) {
      ...
      case Characteristic.RotationSpeed:
        ...
        if (
          ...
          speed = Math.floor(
            (hbValue * this.speed_range.max -
              hbValue * this.speed_range.min +
              100 * this.speed_range.min) /
              100,
          ); //1~100
        }
  ...
  getSpeedFunctionRange(code) {
    ...
    var funcDic = this.functionArr.find((item, index) => {
    ...
    if (funcDic) {
      ...
      return isnull
        ? { min: 1, max: 100 }
        : { min: parseInt(valueRange.min), max: parseInt(valueRange.max) };
    } else {
      return { min: 1, max: 100 };
    }
  }
