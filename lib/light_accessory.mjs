...
class LightAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    // משתני מצב - המקור היחיד לאמת (Source of Truth)
    this.workMode = null;
    this.switchLed = null;
  ...
  initStatus() {
    ...
    if (this._hasBrightness()) {
      service
        ...
        .onGet(() => {
          // אם אנחנו במצב צבע, הבהירות נלקחת מה-V של ה-Color Data
    ...
    if (this.colourData) {
      ...
      service
        ...
        .onSet(async (value) => {
          // עדכון מקומי בלבד - הפקודה תישלח עם ה-Hue (ככה טויה עובדת)
  ...
  async sendTuyaCommand(characteristic, value) {
    ...
    try {
      ...
      // אין יותר צורך ב-setCachedState הישן, ה-updateValue ב-refresh יעשה את העבודה
    } catch (error) {
  ...
  getSendParam(name, value) {
    ...
    switch (name) {
      ...
      case Characteristic.Hue:
        code = this.colourData.code;
        // שולחים אובייקט צבע מלא כדי למנוע איפוסים בטויה
        val = {
          h: value,
          s: this.colourObj.s,
