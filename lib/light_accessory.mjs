...
class LightAccessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (var statusMap of statusArr) {
      ...
      if (
        statusMap.code === "switch_led" ||
        statusMap.code === "switch_led_1"
      ) {
      ...
      if (
        ...
        percentage = Math.floor(
          ((rawValue - this.function_dp_range.bright_range.min) * 100) /
            (this.function_dp_range.bright_range.max -
      ...
      if (
        statusMap.code === "temp_value" ||
        statusMap.code === "temp_value_v2"
      ) {
      ...
      if (
        statusMap.code === "colour_data" ||
        statusMap.code === "colour_data_v2"
      ) {
        ...
        if (!this._isHaveDPCodeOfBrightValue()) {
          var percentage;
          percentage = Math.floor(
            ((this.colourObj.v - this.function_dp_range.bright_range.min) *
              100) /
              (this.function_dp_range.bright_range.max -
                this.function_dp_range.bright_range.min),
          );
        ...
        saturation = Math.floor(
          ((this.colourObj.s - this.function_dp_range.saturation_range.min) *
            100) /
    ...
    switch (name) {
      ...
      case Characteristic.Brightness:
        {
          ...
          if (
            ...
            saturation = Math.floor(
              ((this.function_dp_range.saturation_range.max -
                this.function_dp_range.saturation_range.min) *
                this.getCachedState(Characteristic.Saturation)) /
                100 +
                this.function_dp_range.saturation_range.min,
            ); // value 0~100
      ...
      case Characteristic.Hue:
        ...
        bright = Math.floor(
          ((this.function_dp_range.bright_range.max -
            this.function_dp_range.bright_range.min) *
            this.getCachedState(Characteristic.Brightness)) /
            100 +
            this.function_dp_range.bright_range.min,
        ); //  value 0~100
        saturation = Math.floor(
      ...
      switch (statusMap.code) {
        case "bright_value":
          if (this.deviceCategorie == "dj" || this.deviceCategorie == "dc") {
            ...
            this.deviceCategorie == "xdd" ||
            this.deviceCategorie == "fwd" ||
            this.deviceCategorie == "tgq" ||
            this.deviceCategorie == "dd" ||
            this.deviceCategorie == "tgkg"
          ) {
