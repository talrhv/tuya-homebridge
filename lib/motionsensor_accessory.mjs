...
class MotionSensorAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig, overrideTime) {
    ...
    let timePir = this.statusArr.find((p) => {
      return p.code === "pir_time";
    });
    if (timePir != null) this.timeSensor = true;
    else this.timeSensor = false;
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    if (!isRefresh) this.getAccessoryCharacteristic(this.service);
    for (var statusMap of statusArr) {
      if (statusMap.code === "pir") {
        ...
        if (this.isRefresh) {
          if (this.overrideTime > 0 && !this.timeSensor) {
            if (this.freezeDiscovery === true) return;
      ...
      if (
        statusMap.code === "battery_percentage" ||
        statusMap.code === "battery_value" ||
        statusMap.code === "battery_state"
      ) {
        let rawStatus = statusMap.value;
        switch (true) {
          case statusMap.code === "battery_percentage":
            if (rawStatus > 20) status = 0;
            else status = 1;
            break;
          case statusMap.code === "battery_value":
            if (rawStatus > 2000) status = 0;
            else status = 1;
            break;
          case statusMap.code === "battery_state":
            if (rawStatus === "high" || rawStatus === "middle") status = 0;
            else status = 1;
            break;
        }
  ...
  handleStatusLowBatteryGet() {
    if (
      this.hasValidCache() &&
      this.getCachedState(Characteristic.StatusLowBattery) != null
    )
  ...
  handleStatusTamperedGet() {
    if (
      this.hasValidCache() &&
      this.getCachedState(Characteristic.StatusTampered) != null
    )
