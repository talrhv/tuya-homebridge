...
class TuyaOpenAPI {
  ...
  isLogin() {
    return Boolean(this.tokenInfo?.access_token) && this.tokenInfo.access_token.length > 0;
  }
  ...
  async _refreshAccessTokenIfNeed(path) {
    ...
    const { access_token, refresh_token, uid, expire_time, expire } = res.result ?? {};
  ...
  async getDeviceList() {
    ...
    for (const info of devicesInfoArr) {
      ...
      devices.push(Object.assign({}, info, functions, devicesStatusArr.find((j) => j.id === info.id)));
    }
  ...
  async get_assets() {
    ...
    if (!res?.success) throw new Error(`get_assets failed: ${JSON.stringify(res)}`);
  ...
  async getDeviceIDList(assetID) {
    ...
    if (!res?.success) throw new Error(`getDeviceIDList failed: ${JSON.stringify(res)}`);
  ...
  async getDeviceListStatus(devIds = []) {
    ...
    if (!res?.success) throw new Error(`getDeviceListStatus failed: ${JSON.stringify(res)}`);
    return res.result;
  }
