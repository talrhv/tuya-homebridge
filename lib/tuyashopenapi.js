...
class TuyaSHOpenAPI {
  ...
  async _refreshAccessTokenIfNeed(path) {
    ...
    let res;
    try {
      res = await this.post('/v1.0/iot-01/associated-users/actions/authorized-login', {
  ...
  async getDevices() {
    ...
    if (!res?.success) throw new Error(`getDevices failed: ${JSON.stringify(res)}`);
    ...
    for (const d of res.result.devices) {
      devices.push(Object.assign({}, d, devicesFunctions.find((j) => j.devices?.[0] === d.id)));
    }
  ...
  async getDeviceFunctions(deviceID) {
    ...
    if (!res?.success) throw new Error(`getDeviceFunctions failed: ${JSON.stringify(res)}`);
    return res.result;
  }
  ...
  async getDevicesFunctions(devIds = []) {
    ...
    if (!res?.success) throw new Error(`getDevicesFunctions failed: ${JSON.stringify(res)}`);
    return res.result;
  }
  ...
  async getDeviceInfo(deviceID) {
    ...
    if (!res?.success) throw new Error(`getDeviceInfo failed: ${JSON.stringify(res)}`);
    return res.result;
  }
  ...
  async getDeviceListInfo(devIds = []) {
    ...
    if (!res?.success) throw new Error(`getDeviceListInfo failed: ${JSON.stringify(res)}`);
  ...
  async getDeviceStatus(deviceID) {
    ...
    if (!res?.success) throw new Error(`getDeviceStatus failed: ${JSON.stringify(res)}`);
    return res.result;
  }
  ...
  async removeDevice(deviceID) {
    ...
    if (!res?.success) throw new Error(`removeDevice failed: ${JSON.stringify(res)}`);
    return res.result;
  }
  ...
  async sendCommand(deviceID, params) {
    ...
    if (!res?.success) throw new Error(`sendCommand failed: ${JSON.stringify(res)}`);
    return res.result;
  }
  ...
  async request(method, path, params = null, body = null) {
    ...
    const headers = {
      ...
      sign: this._getSign(this.access_id, this.access_key, access_token, now, nonce, stringToSign),
    ...
    this.log?.log?.(
      `TuyaSHOpenAPI request: ${method.toUpperCase()} ${this.endpoint}${path} params=${JSON.stringify(params)} body=${JSON.stringify(body)}`
    );
    ...
    this.log?.log?.(`TuyaSHOpenAPI response: ${JSON.stringify(res.data)} path=${path}`);
  ...
  _getSign(access_id, access_key, access_token = '', timestamp = 0, nonce, stringToSign) {
    const message = access_id + access_token + `${timestamp}` + nonce + stringToSign;
