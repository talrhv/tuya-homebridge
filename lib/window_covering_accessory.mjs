...
class WindowCoveringAccessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(stateArr, isRefresh) {
    ...
    for (const statusMap of stateArr) {
      ...
      if (statusMap.code === "percent_control") {
        ...
        this.normalAsync(
          Characteristic.TargetPosition,
          this._getCorrectPercent(this.percentControlMap.value),
        );
      ...
      if (statusMap.code === "position") {
        this.percentControlMap = statusMap;
        const percent = this._getCorrectPercent(
          parseInt(this.percentControlMap.value),
        );
      ...
      if (statusMap.code === "percent_state") {
        ...
        this.normalAsync(
          Characteristic.CurrentPosition,
          this._getCorrectPercent(this.positionMap.value),
        );
  ...
  _isHaveDPCodeOfPercentState() {
    const percentStateDic = this.statusArr.find((item, index) => {
      return item.code.indexOf("percent_state") != -1;
    });
    if (percentStateDic) {
      return true;
    } else {
