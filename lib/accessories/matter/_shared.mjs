...
export const SWITCH_CODES = ["switch", "switch_1", "switch_2", "switch_3", "switch_4", "switch_5", "switch_6", "switch_7", "switch_8"];
export const COUNTDOWN_CODES = ["countdown", "countdown_1", "countdown_2", "countdown_3", "countdown_4", "countdown_5", "countdown_6", "countdown_7", "countdown_8"];
export const BRIGHTNESS_CODES = ["bright_value_v2", "bright_value", "bright_value_1"];
...
export function extractStatusEntries(source) {
  ...
  if (Array.isArray(source.data?.status)) return source.data.status.filter(Boolean);
  if (Array.isArray(source.statusList)) return source.statusList.filter(Boolean);
  if (Array.isArray(source.bizData?.status)) return source.bizData.status.filter(Boolean);
  return [];
}
...
export function hasCode(device, code) {
  ...
  if (extractStatusEntries(device).some((entry) => entry?.code === code)) return true;
...
export function toBoolean(value, fallback = false) {
  ...
  if (typeof value === "string") {
    return ["true", "1", "on", "opened", "open", "pir", "alarm", "detected"].includes(value.toLowerCase());
  }
  return fallback;
}
...
export function readBrightnessPercent(source, code) {
  ...
  return rangeToPercent(getStatusValue(source, code || BRIGHTNESS_CODES), range);
}

export function readColorTempPercent(source, code) {
  const range = getNumericRangeForCode(source, code, 0, 1000);
  return rangeToPercent(getStatusValue(source, code || COLOR_TEMP_CODES), range);
}
...
export function readContactOpen(source) {
  const value = getStatusValue(source, ["doorcontact_state", "contact_state", "door_open"]);
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value > 0;
  if (typeof value === "string") {
...
export function readLeakDetected(source) {
  const value = getStatusValue(source, ["gas_sensor_status", "gas_sensor_state", "ch4_sensor_state", "watersensor_state", "watersensor_status", "leak_state", "sensor_state"]);
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value > 0;
  if (typeof value === "string") {
    return ["alarm", "warn", "warning", "true", "1", "leak", "detected", "wet"].includes(value.toLowerCase());
  }
  return false;
}

export function readSmokeDetected(source) {
  const value = getStatusValue(source, ["smoke_sensor_status", "smoke_sensor_state", "smoke_state", "smoke_sensor_status", "smoke_alarm"]);
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value > 0;
  if (typeof value === "string") {
    return ["alarm", "warn", "warning", "true", "1", "detected", "smoke"].includes(value.toLowerCase());
  }
  return false;
}

export function readMotionDetected(source) {
  const value = getStatusValue(source, ["pir", "pir_state", "presence_state", "motion_state"]);
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value > 0;
  if (typeof value === "string") {
    return ["pir", "presence", "motion", "alarm", "detected", "true", "1"].includes(value.toLowerCase());
  }
  return false;
}
...
export function sortGangCodes(codes = []) {
  return [...codes].sort((left, right) => {
    ...
    const rightNum = right === "switch" ? 0 : Number(right.split("_")[1] || 999);
...
export function getGangCodes(device, prefix = "switch") {
  ...
  for (const entry of extractStatusEntries(device)) {
    if (typeof entry?.code === "string" && entry.code === prefix || new RegExp(`^${prefix}_[0-9]+$`).test(entry?.code || "")) {
...
export function getCountdownCodes(device) {
  const codes = new Set();
  for (const entry of extractStatusEntries(device)) {
    if (typeof entry?.code === "string" && (entry.code === "countdown" || /^countdown_[0-9]+$/.test(entry.code))) {
      codes.add(entry.code);
    }
  }
  for (const entry of listFunctions(device)) {
...
export function readWindowOpenPercent(device, context = {}) {
  ...
  const value = Number(getStatusValue(device, context?.positionCode || ["percent_state", "percent_control", "position"]));
  if (!Number.isFinite(value)) return 0;
  return fullyOpenDefault ? value : 100 - value;
}

export function windowOpenPercentToMatterClosedPercent100ths(openPercent) {
  return Math.round((100 - Math.max(0, Math.min(100, Number(openPercent) || 0))) * 100);
}
